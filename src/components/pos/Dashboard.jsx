...
export default function Dashboard({ user, session, lastSessionInfo, onOpenSessionClick, onLockScreen, onProceedToPOS }) {
    ...
    <div className="flex flex-col h-screen w-full bg-surface-base overflow-hidden relative font-sans text-text-primary">
      ...
      <header className="px-8 py-5 flex items-center justify-between z-10 bg-white/50 backdrop-blur-xl border-b border-border/50">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-3">
            <h1 className="text-xl font-black tracking-tight leading-none">Odoo Cafe POS</h1>
            <span className="px-2 py-0.5 bg-success-100 text-success-700 text-[10px] font-black rounded-full uppercase tracking-widest">Live v2</span>
          </div>
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2 px-3 py-1.5 bg-surface-hover rounded-full border border-border">
            <div className="w-6 h-6 rounded-full bg-gradient-to-tr from-amber-400 to-amber-600 shadow-sm flex items-center justify-center text-[10px] font-black text-white uppercase">
        ...
        {session.status === 'open' ? (
            ...
            >
               <div className="text-center">
                 <div className="inline-flex items-center justify-center p-6 bg-success-50 text-success-600 rounded-full mb-8 ring-8 ring-success-50/50">
                    <CheckCircle2 className="w-12 h-12" />
                 </div>
                 <h2 className="text-5xl font-black text-text-primary mb-4 tracking-tight">Session is Active</h2>
                 <p className="text-xl text-text-secondary mb-12">Terminal is ready for processing orders.</p>
                 
                 <button 
                   onClick={onProceedToPOS}
                   className="group flex items-center gap-4 px-10 py-5 bg-primary-600 text-white rounded-[2rem] font-black text-xl shadow-xl shadow-primary-600/30 transition-all hover:scale-[1.02] hover:bg-primary-700 active:scale-95"
                 >
                   <span>Proceed to Floor Plan</span>
            ...
            >
               <div className="text-center mb-12">
                 ...
                 <p className="text-lg text-text-secondary">
                   Please open a new session to begin taking orders and processing payments.
                 </p>
               </div>
                 ...
                 <div className="bg-white rounded-3xl p-6 shadow-xl border border-border/60 mb-10 overflow-hidden relative">
                   ...
                   <div className="grid grid-cols-2 gap-8">
                     <div>
                       ...
                       <p className="text-3xl font-black tabular-nums font-mono text-primary-700">₹{(lastSessionInfo.totalSales || 0).toFixed(2)}</p>
                     </div>
                     {isAdmin ? (
                       <div>
                         <div className="flex items-center gap-2 text-text-secondary mb-1">
                         ...
                         <p className="text-3xl font-black tabular-nums font-mono">₹{(lastSessionInfo.closingBalance || 0).toFixed(2)}</p>
                       </div>
                     ) : (
                       <div className="flex items-center justify-center bg-slate-50 rounded-2xl border border-dashed border-slate-200">
                         <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest text-center px-4">Financial details restricted to Admin</span>
                       </div>
                     )}
                   </div>
                 </div>
