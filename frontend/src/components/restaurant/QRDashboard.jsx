      ...
      <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
        {Object.entries(
          filteredTables.reduce((acc, table) => {
            const floor = table.floor || 'unassigned';
            if (!acc[floor]) acc[floor] = [];
            acc[floor].push(table);
            return acc;
          }, {})
        ).map(([floor, floorTables]) => (
          <div key={floor} className="mb-12 print:mb-8">
            <h2 className="text-xl font-black text-slate-800 uppercase tracking-widest mb-6 border-b border-slate-200 pb-2 capitalize">
              {floor} Floor
            </h2>
